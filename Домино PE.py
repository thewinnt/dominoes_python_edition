...
try:
    config = open("domino_config.dc2", 'r', -1, 'utf-8') # check is the file is missing and if it is then make a new one with default settings
except OSError:
    print("File error: no config file present, creating a new one")
    config = open("domino_config.dc2", 'w', -1, 'utf-8')
    config.seek(0, 0)
    config.write('1\n0\n1')
config = open("domino_config.dc2", 'r', -1, 'utf-8')
config.seek(0, 0)
...
config = open("domino_config.dc2", 'r', -1, 'utf-8') # read the themes
config.seek(0, 0)
config_data = config.readlines()
...
theme_list = default_themes + custom_themes # a list of all available themes
...
def next_player():
    ...
    if not fastmode and gamemode == "local":
        ...
        # draw all the texts
        window_title = font_header.render("Ожидание след. игрока", 4, current_theme[11])
...
def settings(source):
    ...
    go_back = font_gm_sel.render("Настройки темы...", 4, current_theme[11])
    window.blit(go_back, (40, 266))
    font_gm_sel.set_underline(False)
    mp = pygame.mouse.get_pos()
    if mp[0] > 40 and mp[0] < 40 + go_back.get_width() and mp[1] > 266 and mp[1] < 258 + go_back.get_height() and is_clicc:
    ...
    if source == "pause":
        titl = font_header.render("Игра на паузе", 4, current_theme[11]) # typo intentional
    else:
        titl = font_header.render("Настройки", 4, current_theme[11])
...
##    # draw the window
##    window.fill(current_theme[8])
##    pygame.draw.rect(window, current_theme[10], (250, 50, 780, 620))
##    pygame.draw.rect(window, current_theme[0], (250, 50, 780, 620), 4)
##    pygame.draw.rect(window, current_theme[8], (250, 50, 780, 40))
##    pygame.draw.rect(window, current_theme[0], (250, 50, 780, 40), 4)
##    # draw the texts
##    window_title = font_d50.render("Настройка", 4, current_theme[11])
##    window.blit(window_title, (640 - (window_title.get_width() / 2), 70 - (window_title.get_height() / 2)))
##    score_limit = font_setup.render("Лимит очков", 4, current_theme[11])
##    window.blit(score_limit, (300, 120))
##    txt_min_write = font_setup.render("Начало записи", 4, current_theme[11])
##    window.blit(score_limit, (300, 120))
##    txt_player_size = font_setup.render("Фишек у игроков", 4, current_theme[11])
##    window.blit(score_limit, (300, 120))
##    # render the rest AND detect input in a loop
...
def theme_set_ui(source="settings"): # a sub-ui inside the settings menu - here you can set your theme and import new ones
    ...
    title123 = font_header.render("Выбор темы", 4, current_theme[11])
    ...
    title123 = font_gm_sel.render("Выберите тему из списка:", 4, current_theme[11])
    theme_surface.blit(title123, (20, 50))
    title123 = font_ver.render("Выбранная тема:", 4, current_theme[11])
    theme_surface.blit(title123, (550, 160))
    title123 = font_ver.render("Больше тем:", 4, current_theme[11])
    ...
    blit_multiline(theme_surface, current_theme[16] + ' · ' + current_theme[18], (550, 240), 720, font_fps, current_theme[11])
    pos = pygame.mouse.get_pos()
    win_pos = [pos[0] - 250, pos[1] - 50]
    inner_surface.fill(current_theme[8])
    ...
    for i in range(len(theme_list)):
        ...
        if pos[0] > 275 and pos[0] < 275+width and pos[1] > 190+scroll_offset+150*i and pos[1] < 330+scroll_offset+150*i:
            if not test_button.isOver(in_pos) and not demo_slider.is_over(in_pos) and pygame.mouse.get_pressed()[0]:
...
while True: # game tick, running at 60 TPS
    ...
    if bool(pygame.event.peek(pygame.QUIT)):
        with open("domino_config.dc2", 'w', -1, 'utf-8') as config:
            final_config_data = []
            for i in range(len(config_data)):
                if not config_data[i].endswith('\n'):
                    config_data[i] += '\n'
                final_config_data.append(config_data[i])
            final_config_data[-1] = final_config_data[-1].replace('\n', '')
            config.writelines(final_config_data) # the settings are saved only when you close the game properly
        pygame.quit()
        break
