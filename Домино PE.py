...
pygame.display.set_caption("Домино: Python Edition v. Beta 1.7")
game_version = "v. Beta 1.7"
    ...
    def __init__(self, settings, ui_hooks, mid_game={'initialized': False}, filename=None):
        ...
        if mid_game['initialized']: # a set of values that appear after starting the game
            ...
            for i in range(self.player_count):
                if self.is_ai(i+1):
                    self.player_names.append('Бот ' + str(b))
                    b += 1
                else:
                    self.player_names.append('Игрок ' + str(p))
                    p += 1
...
class ui: # the game's GUI
    def __init__(self, surface, init=True):
        ...
        self.btn_continue = ui_tools.Button(self.surface, 480, 420, 155, 55, font_d60, "Продолжить", current_theme[14], current_theme[12], current_theme[13], current_theme[15], current_theme[0])
        self.btn_load_game = ui_tools.Button(self.surface, 645, 420, 155, 55, font_d60, "Загрузить", current_theme[14], current_theme[12], current_theme[13], current_theme[15], current_theme[0])
        self.btn_new_game = ui_tools.Button(self.surface, 480, 355, 320, 55, font_d60, "Новая игра", current_theme[14], current_theme[12], current_theme[13], current_theme[15], current_theme[0])
        self.btn_settings = ui_tools.Button(self.surface, 480, 485, 320, 55, font_d60, "Настройки", current_theme[14], current_theme[12], current_theme[13], current_theme[15], current_theme[0])
        self.btn_how_to_play = ui_tools.Button(self.surface, 480, 550, 155, 55, font_d60, "Как играть", current_theme[14], current_theme[12], current_theme[13], current_theme[15], current_theme[0])
        self.btn_exit = ui_tools.Button(self.surface, 645, 550, 155, 55, font_d60, 'Выйти', current_theme[14], current_theme[12], current_theme[13], current_theme[15], current_theme[0])
        ...
        self.btn_gm_internet = ui_tools.Button(self.surface, 250, 500, 300, 55, font_d60, "В разработке...", current_theme[14], current_theme[12], current_theme[13], current_theme[16], current_theme[16])
        ...
        self.exit_game = ui_tools.Button(self.surface, 990, 630, 250, 50, font_d60, 'Выйти в меню', current_theme[14], current_theme[12], current_theme[13], current_theme[15], current_theme[0])
        self.save_game = ui_tools.Button(self.surface, 730, 630, 250, 50, font_d60, 'Сохранить и выйти', current_theme[14], current_theme[12], current_theme[13], current_theme[15], current_theme[0])
    ...
    def render_main_menu(self, btn=True):
        ...
        if btn:
            ...
            if is_game_running:
                ...
                self.btn_load_game.text = 'Загрузить'
                ...
                self.btn_load_game.text = 'Загрузить игру'
            # new game button
            ...
            if self.btn_load_game.draw() and pygame.event.get(pygame.MOUSEBUTTONDOWN):
                file = filedialog.askopenfilename(title='Загрузить игру', filetypes=[('Файл JSON', '*.json'), ('Все файлы', '*.*')])
                if file:
        ...
        if len(p) == 4:
            for i in range(len(p)):
                ...
                if i == 0:
                    ...
                    playerdata = font_pd.render(f"{p[i]['name']} - {str(p[i]['score'])} очков, {str(p[i]['victories'])} побед", 4, current_theme[15], current_theme[13 - int(self.game.current_move == i + 1)])
                ...
                # |
                    ...
                    score = font_pd.render(f"{str(p[i]['score'])} очков", 4, current_theme[15], current_theme[13 - int(self.game.current_move == i + 1)])
                    vict = font_pd.render(f"{str(p[i]['victories'])} побед", 4, current_theme[15], current_theme[13 - int(self.game.current_move == i + 1)])
            ...
            playerdata = font_pd.render(f"{p[0]['name']} - {str(p[0]['score'])} очков, {str(p[0]['victories'])} побед", 4, current_theme[15], current_theme[13 - int(self.game.current_move == i + 1)])
            ...
            playerdata = font_pd.render(f"{p[1]['name']} - {str(p[1]['score'])} очков, {str(p[1]['victories'])} побед", 4, current_theme[15], current_theme[13 - int(self.game.current_move == i + 1)])
    ...
    def settings(self):
        ...
        if current_ui == "pause":
            ...
            if self.save_game.draw() and pygame.event.get(pygame.MOUSEBUTTONDOWN):
                ...
                filename = filedialog.asksaveasfilename(defaultextension = '.json', filetypes = [('Файлы JSON', '*.json'), ('Все файлы', '*.*')], title = 'Сохранить игру')
                if filename:
    ...
    def game_setup(self):
        ...
        error_messages = ['',
                        ...
                        f"У игроков должно быть не более {28//(player_count)} фишек, иначе их не хватит на всех",
                        "Игроков может быть не больше 4",
                        "Игроков может быть хотя бы двое"
                        ]
        ...
        txt_begin_with_double = font_setup.render("Количество игроков", 4, current_theme[15])
