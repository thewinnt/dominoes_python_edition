...
pygame.display.set_caption("Домино: Python Edition v. Beta 1.6.2")
game_version = "v. Beta 1.6.2"
...
btn_play = button(current_theme[10], 480, 400, 320, 50, "Играть")
btn_settings = button(current_theme[10], 480, 470, 320, 50, "Настройки")
btn_how_to_play = button(current_theme[9], 480, 540, 155, 50, "Как играть")
btn_exit = button(current_theme[10], 645, 540, 155, 50, 'Выйти')
...
def settings(source):
    ...
    opt4 = font_gm_sel.render("Звук установки доминошки", 4, current_theme[11])
    window.blit(opt4, (40, 266))
    opt5 = font_gm_sel.render("Звук ошибки", 4, current_theme[11])
    window.blit(opt5, (40, 336))
    if source == "pause":
        titl = font_header.render("Игра на паузе", 4, current_theme[11]) # typo intentional
        # back to main menu button
        btgs = button(current_theme[10], 1040, 630, 200, 50, 'Выйти в меню')
        if btgs.isOver():
    ...
    opt4 = font_gm_sel.render("Время мерцания счёта при победе (в секундах)", 4, current_theme[11])
...
def theme_set_ui(source="settings"): # a sub-ui inside the settings menu - here you can set your theme and import new ones
    ...
    for i in range(len(theme_list)):
        ...
        btn_del = button(theme_list[i][10], 370, 15+scroll_offset+150*i, 80, 25, 'Удалить')
        if i > 1:
...
def game_setup():
    ...
    while not is_finished:
        ...
        txt_score_limit = font_setup.render("Лимит очков", 4, current_theme[11])
        ...
        txt_begin_with_double = font_setup.render("Начинать новую игру с дубля", 4, current_theme[11])
        ...
        button_exit = button(current_theme[10], 590, 550, 170, 50, 'Начать игру')
        ...
        button_back = button(current_theme[10], 400, 550, 170, 50, 'Назад')
...
def how_to_play():
    while True:
        ...
        titl = font_ver.render("Как играть", 4, current_theme[11])
        tutorial.blit(titl, (250 - titl.get_width() / 2, 0))
        in_pos = pygame.mouse.get_pos()
        in_pos = [in_pos[0] - 390, in_pos[1] - 150]
        button_back = button(current_theme[10], 175, 390, 150, 50, 'Назад')
...
while True: # game tick, running at 60 TPS
    ...
    if is_theme_updated:
        btn_play = button(current_theme[10], 480, 400, 320, 50, "Играть")
        btn_settings = button(current_theme[10], 480, 470, 320, 50, "Настройки")
        btn_how_to_play = button(current_theme[10], 480, 540, 155, 50, "Как играть")
        btn_exit = button(current_theme[10], 645, 540, 155, 50, 'Выйти')
        btn_gm_local = button(current_theme[10], 250, 400, 300, 50, "Локальная игра")
        btn_gm_internet = button(current_theme[9], 250, 500, 300, 50, "В разработке...")
        btn_gm_pva = button(current_theme[10], 730, 400, 300, 50, "Одиночная игра")
